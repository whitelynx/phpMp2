...
<?php
	...
	if (array_key_exists("content", $_REQUEST)) echo "<meta name=\"content\" content=\"".$_REQUEST["content"]."\" />";
	if (array_key_exists("directory", $configuration)) echo "<meta name=\"directory\" content=\"".$configuration["directory"]."\" />";
	echo "<style type=\"text/css\" media=\"screen\">\n<!--\nhtml { font-size: ".$configuration["font_size"]." }\n-->\n</style>\n";
	if(($layout_vars["frames"] == false || (array_key_exists("content", $_REQUEST) && in_array($_REQUEST["content"], $layout_vars["auto-reload"]))) && $configuration["auto_refresh"] == true)
		echo "<meta http-equiv=\"REFRESH\" content=\"".$configuration["refresh_freq"].";URL=index.php".(array_key_exists("content", $_REQUEST) ? "?content=".$_REQUEST["content"] : "")."\" />\n";

	if($handle = opendir("./styles")) {
		$dirs = array();
		while( $file = readdir( $handle ) ) {
			if($file != "." && $file != ".." && is_dir("./styles/".$file) && file_exists("./styles/".$file."/style.css")) {
				$dirs[] = $file;
			}
		}
		closedir( $handle );
		foreach($dirs as $key => $dir) {
			if($configuration["style"] == $dir) {
				echo "<link rel=\"StyleSheet\" href=\"styles/".$dir."/style.css\" type=\"text/css\" title=\"".$dir."\" />\n";
				...
				echo "<link rel=\"Alternate StyleSheet\" href=\"styles/".$dir."/style.css\" type=\"text/css\" title=\"".$dir."\" />\n";
			}
		}
	}
}
...
<?php
	...
	if($noconnect != true) fclose($connection);
} else {
?>
